...
Reconciliation Breaks Engine (F1): Deterministic multi-pass reconciliation matcher with a ranked break worklist and drafted notes.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `recon_breaks_engine`, environment prefix
`RECONBREAKS`, region `asia-southeast1`.
...
- `src/recon_breaks_engine/domain/` is PURE stdlib. No web framework, no cloud SDK,
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `RECONBREAKS_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `RECONBREAKS_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `RECONBREAKS_PROFILE` (mentioning it in a refusal message is wanted, reading
...
- **Cross-tenant reads are refused in the DOMAIN, 403 not 404.** The `WorklistStorePort` is
  tenant-scoped: `list_for_tenant` filters on tenant in the store so a listing can never span
  tenants, and `get` is a raw fetch that `worklist_access.read_worklist` authorizes against the
  VERIFIED principal's tenant (never a client-asserted one). A worklist owned by another tenant is
  a 403 (the record exists, the caller may not see it), because a 404 would leak whether an id is
  in use across the boundary. `tests/unit/test_worklist_store_authz.py` proves the raw `get` leaks
  without the domain check, so the guard can go red.
