# SPEC: Reconciliation Breaks Engine (F1)
...
- **Maker-checker (P-06) and routing (R8)**: a HIGH/CRITICAL result sets
  ...
  same request. The flag alone is not the escalation. The response carries `review_routing`
  (`routed`, `failed`, `off` or `not_required`), so a caller can tell a routed escalation from
  one that stopped here. Under the managed profile a deployment with no console configured
  refuses at boot; a hand-off that fails at request time is reported as `failed` and logged
  rather than failing the run. `RECONBREAKS_REVIEW_ROUTING` switches routing (default on).
- **Profile**: resolved ONCE, at import, into a `ProfileChoice` and never a bare string. Three
  states of `RECONBREAKS_PROFILE`: UNSET is NO CHOICE (the SDK-free adapters
