"""FastAPI application for Reconciliation Breaks Engine (F1).
...
``RECONBREAKS_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``RECONBREAKS_S2S_TOKEN`` is wrong in the most dangerous direction:
...
from ..domain.kernel import TenantAccessDeniedError
...
from ..worklist_access import read_worklist
from .schemas import (
    HealthResponse,
    ReconcileRequest,
    ReconcileResponse,
...
_INSECURE_DEMO_ENV = "RECONBREAKS_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "RECONBREAKS_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "RECONBREAKS_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "RECONBREAKS_S2S_AUDIENCE"
_API_HOST_ENV = "RECONBREAKS_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``RECONBREAKS_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``RECONBREAKS_PROFILE`` must never be allowed to
    ...
    ``RECONBREAKS_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "RECONBREAKS_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "recon-breaks-engine"
...
_APP_TITLE = "Reconciliation Breaks Engine"
...
#
# Note what is NOT in this expression: RECONBREAKS_S2S_TOKEN. A service
...
@app.post("/v1/reconcile", response_model=ReconcileResponse, tags=["artifacts"])
def reconcile(
    ...
    as_of = date.fromisoformat(request.as_of) if request.as_of else date.fromisoformat(today_iso())
    run = service.run(
        feed_a=request.feed_a,
        feed_b=request.feed_b,
        as_of=as_of,
        actor=principal.actor,
        tenant=principal.tenant,
    )
    # Persist the ranked worklist under a tenant-scoped handle so it can be retrieved later. The
    # tenant is the VERIFIED principal's, never anything the client wrote; the id embeds it so the
    # store partitions by tenant and the retrieval path can authorize the read (403, not 404).
    feed_id = f"{request.feed_a}:{request.feed_b}"
    worklist_id = f"wl:{principal.tenant}:{feed_id}"
    container.worklist_store.put(
        StoredWorklist(
...
def worklist(
    ...
    """Retrieve a persisted ranked worklist, authorizing the tenant in the DOMAIN (403, not 404).

    ``read_worklist`` fetches the worklist by id and compares its stored tenant to the VERIFIED
    principal's tenant: a worklist owned by another tenant is refused with 403 (the record exists,
    the caller may not see it), and a worklist that does not exist is a 404. The client-asserted
    actor is never read; the tenant is the one the identity adapter verified for this request.
    """
    try:
        stored = read_worklist(_container(), worklist_id, principal_tenant=principal.tenant)
    except TenantAccessDeniedError as exc:
        raise HTTPException(
            status_code=exc.http_status, detail="cross-tenant access denied"
        ) from exc
    if stored is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="worklist not found")
    return WorklistResponse.from_domain(stored)
