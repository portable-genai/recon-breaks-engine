...
One env var (``RECONBREAKS_PROFILE``) selects the adapter family for every
...
from .ports.worklist_store import WorklistStorePort

_PROFILE_ENV = "RECONBREAKS_PROFILE"
_SETTINGS_ENV = "RECONBREAKS_SETTINGS"
...
class ProfileChoice:
    ...
    The variable is ``RECONBREAKS_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("RECONBREAKS_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "feeds": {
        "local": f"{_PKG}.adapters.local.feeds:LocalFeedAdapter",
        "gcp": f"{_PKG}.adapters.gcp.feeds:CloudFeedAdapter",
        "onprem": f"{_PKG}.adapters.onprem.feeds:OnPremFeedAdapter",
    },
    "generation": {
    ...
    "case_engine": {
        "local": f"{_PKG}.adapters.local.case_engine:LocalCaseRecorder",
        "gcp": f"{_PKG}.adapters.gcp.case_engine:CloudCaseEngine",
        "onprem": f"{_PKG}.adapters.onprem.case_engine:OnPremCaseEngine",
    },
    "worklist_store": {
        "local": f"{_PKG}.adapters.local.worklist_store:LocalWorklistStoreAdapter",
        "gcp": f"{_PKG}.adapters.gcp.worklist_store:CloudWorklistStoreAdapter",
        "onprem": f"{_PKG}.adapters.onprem.worklist_store:OnPremWorklistStoreAdapter",
    },
}
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``RECONBREAKS_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "RECONBREAKS_REVIEW_ROUTING"
...
def _refuse_unconfigured_controls(settings: Settings) -> None:
    """The managed profile must name its console, checked at boot.

    The managed router and the managed case engine both used to discover a missing
    ``review_url`` on the first escalation and fail that request; the configuration error
    belongs at startup. Switching review routing off does NOT make the console optional here:
    the case engine opens every breaching break's escalation case on the same console, and it
    is not a switchable control.
    """
    if settings.profile not in _MANAGED_PROFILES or settings.review_url.strip():
        return
    if settings.controls.review_routing:
        raise ConfiguredEmptyError(
            ...
            f"URL; {REVIEW_ROUTING_ENV}=off alone is not enough here, because the case engine "
            f"opens escalation cases on the same console."
        )
    raise ConfiguredEmptyError(
        f"{REVIEW_ROUTING_ENV} is off under profile {settings.profile!r}, but HUMAN_REVIEW_URL "
        f"(config/settings.yaml review_url) is still required: the case engine opens every "
        f"breaching break's escalation case on the human-review-console."
    )
...
class Container:
    ...
    @cached_property
    def worklist_store(self) -> WorklistStorePort:
        adapter = self._bind("worklist_store")
        assert isinstance(adapter, WorklistStorePort)
        return adapter
