...
``RECONBREAKS_S2S_TOKEN`` used to switch the guard off under a DELIBERATE
...
from recon_breaks_engine.api.app import (
    app,
)
...
_IAP_AUDIENCE_ENV = "RECONBREAKS_IAP_AUDIENCE"
...
def test_the_zero_secret_demo_is_bounded_exactly_as_the_token_bearing_one_is(
    ...
    resp = _call(_client(LAN_PEER, app_without_token.app), "POST", "/v1/reconcile")
    assert resp.status_code == 503
...
def test_the_local_demo_still_works_on_loopback_with_a_service_token_set(
    ...
    resp = _call(_client("127.0.0.1", local_with_service_token.app), "POST", "/v1/reconcile")
...
def test_but_its_end_user_route_still_refuses_an_uncredentialed_lan_peer(
    ...
    """
    resp = _call(_client(LAN_PEER, verifying_identity.app), "POST", "/v1/reconcile")
    assert resp.status_code == 503
...
def test_a_configured_verifying_profile_answers_401_to_a_peer_with_no_assertion(
    ...
    resp = _call(_client(LAN_PEER, module.app), "POST", "/v1/reconcile")
    assert resp.status_code == 401
...
def test_an_unimplemented_identity_binding_refuses_a_lan_peer(
    ...
    resp = _call(_client(LAN_PEER, unimplemented_identity.app), "POST", "/v1/reconcile")
    assert resp.status_code == 503
...
def test_an_unimplemented_identity_binding_answers_a_status_and_a_reason_not_a_bare_500(
    ...
    """
    resp = _call(_client("127.0.0.1", unimplemented_identity.app), "POST", "/v1/reconcile")
