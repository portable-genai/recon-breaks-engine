...
from recon_breaks_engine.adapters.local.audit import (
    LocalAuditAdapter,
)
...
from recon_breaks_engine.domain.kernel import AuditEvent, Citation, Decision, Severity

from tests.fixtures import sample_cases

_ANCHOR_ENV = "RECONBREAKS_AUDIT_ANCHOR"
...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    One reconciliation run writes one already-redacted audit event per break, which is more than
    the three the truncation proofs below need.
    """
...
def _one_more_event(audit: LocalAuditAdapter) -> None:
    """Append a single already-redacted audit event directly through the sink."""
    audit.record(
        AuditEvent(
            ...
            redacted_summary="one more redacted break summary",
            citations=(Citation(source_id="nostro:1", title="line", snippet="x"),),
        )
    )
