...
from recon_breaks_engine.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from recon_breaks_engine.config import Settings
from recon_breaks_engine.ports.identity import (
    VERIFIED,
...
_API_MODULE = "recon_breaks_engine.api.app"
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        # The three DATA ports the reconcile route needs, bound to the SDK-free family under gcp
        # too, so this app exercises the real gcp IDENTITY adapter while staying offline: no
        # BigQuery, no model endpoint, no live case engine. Settings.load rejects a partial
        # adapters block, so every port the domain declares must appear here.
        "  feeds:",
        *[f"    {p}: {_PKG}.adapters.local.feeds:LocalFeedAdapter" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.feeds:OnPremFeedAdapter",
        "  generation:",
        *[
        ...
        "  case_engine:",
        *[
            f"    {p}: {_PKG}.adapters.local.case_engine:LocalCaseRecorder"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.case_engine:OnPremCaseEngine",
        "  worklist_store:",
        *[
            f"    {p}: {_PKG}.adapters.local.worklist_store:LocalWorklistStoreAdapter"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.worklist_store:OnPremWorklistStoreAdapter",
    ]
)
...
def test_a_malformed_assertion_from_a_lan_peer_answers_401_and_never_500(
    ...
    with TestClient(module.app, client=(LAN_PEER, 50000)) as client:
        resp = client.post(
            "/v1/reconcile",
            json={"feed_a": "nostro", "feed_b": "scheme"},
