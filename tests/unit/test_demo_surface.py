...
def test_the_demo_data_is_obviously_fictional() -> None:
    """Every party the reconciliation demo shows is invented, and a real one trips this test.

    The demo reconciles feeds rather than triaging incident text, so the fictional markers are
    the ones that fit that shape: a ``.example`` verified principal, an invented counterparty
    roster (a real institution added to the feeds or the PII row fails the equality below rather
    than shipping in a demo), and the documented synthetic national id that exists only so the
    redaction beat has an independent literal to prove it masked. There is no routable-address
    literal to assert here because a reconciliation feed carries amounts and counterparties, not
    IPs; the roster allowlist is the equivalent tripwire.
    """
    from recon_breaks_engine.adapters.local._recon_fixture import FEED_A, FEED_B, feed_rows

    # The verified principal is on a documentation domain, never a real or consumer mailbox.
    assert demo.ACTOR.endswith(".example"), demo.ACTOR + " is not on a .example domain"

    # Every counterparty the engine reconciles, plus the PII-carrying row, is drawn from the
    # invented roster. Equality (not membership) is deliberate: an UNLISTED name, real or not,
    # fails until a maintainer confirms it is fictional and adds it here.
    counterparties = {row.counterparty for row in feed_rows(FEED_A)} | {
        row.counterparty for row in feed_rows(FEED_B)
    }
    invented = {
        "Acme Ltd",
        "Beta Trading",
        ...
        "Iota Co",
        "Kappa Ltd",
        "Omega Nominees NRIC " + demo.PLANTED_NRIC,
    }
    assert counterparties == invented, "an unlisted counterparty appeared: " + str(
        counterparties.symmetric_difference(invented)
    )

    # The planted identifier is the documented synthetic literal, present only to prove redaction.
    assert demo.PLANTED_NRIC == "S1234567D"

    # No real-looking consumer mailbox anywhere in the whole rendered run, nor in the identities.
    run = demo.DemoRun()
    run.run_to_end()
    blob = json.dumps(run.state(), sort_keys=True) + demo.ACTOR + demo.TENANT
