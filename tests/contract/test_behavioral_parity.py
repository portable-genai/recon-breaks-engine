...
from recon_breaks_engine.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    demo.
    """
    resolution = sample_cases.pii_resolution()
    payload = repr(
        result_to_review(
            resolution, maker=sample_cases.ACTOR, tenant=sample_cases.TENANT
        ).to_payload()
    )
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    run = sample_cases.build_service(local).run(
    ...
    assert run.ranked_breaks, "an offline answer must produce the ranked worklist"
    assert run.matches and all(m.citations for m in run.matches), "every match must be cited"

    onprem = build_container(local_settings(profile="onprem"))
    with pytest.raises(NotImplementedError):
        sample_cases.build_service(onprem).run(
            feed_a=sample_cases.NOSTRO_FEED,
            feed_b=sample_cases.SCHEME_FEED,
            as_of=sample_cases.AS_OF,
            actor=sample_cases.ACTOR,
