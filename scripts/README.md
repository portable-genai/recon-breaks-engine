...
One entry in the table below, `rename_fork.py`, is a fork tool rather than a demo script. It sits
here because it obeys the same two rules (stdlib only, no network) and because the index test
above requires every `scripts/*.py` to be described in one place; its row says what it is.
...
| `rename_fork.py` | NOT a demo script: the one-pass mechanical rebrand a FORK runs once, rewriting the package name, the console-script name, the `RECONBREAKS` env prefix, the Terraform resource stem and the distribution id. It lives here because it is stdlib-only and offline like everything else in this directory, and because `docs/ADOPTING.md` is the only document that calls it. It has no `make` target on purpose: a rebrand is a deliberate one-time act, not a repeatable target somebody runs by reflex. |
