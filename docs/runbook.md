# Runbook: Reconciliation Breaks Engine (F1)

## Deploy (gcp)
1. `RECONBREAKS_PROFILE=gcp`, install `.[gcp]`, region `asia-southeast1`.
...
3. Ingress is fronted by IAP/Apigee; the app authenticates the S2S caller fail-closed
   (`RECONBREAKS_S2S_TOKEN` local, Google OIDC + allowlist secure).
4. **Set `RECONBREAKS_IAP_AUDIENCE`.** Without it the service starts, stays
...
`RECONBREAKS_IAP_AUDIENCE` is the IAP-protected resource the assertion must be
...
| Symptom | Cause | Fix |
|---|---|---|
| 503, detail names `RECONBREAKS_IAP_AUDIENCE` | no audience configured | set it to the protected resource above |
...
| `gcp` IAP assertion | `verified` | authenticated: the signature (against IAP's own key set), the audience (against `RECONBREAKS_IAP_AUDIENCE`), the expiry and the issuer are all checked before any claim is read |
...
1. **Nobody chose a profile.** `RECONBREAKS_PROFILE` is absent, so no end-user
...
2. **The `local` profile, chosen deliberately.** The seeded personas are a client-asserted
   ...
   `RECONBREAKS_S2S_TOKEN` is set. Setting that secret closes the S2S
...
- `add_loopback_exposure_guard` is registered at module scope in `api/app.py`, so it holds under
  `uvicorn recon_breaks_engine.api.app:app --host 0.0.0.0` (what the Dockerfile `CMD`
...
Set `RECONBREAKS_ALLOW_INSECURE_DEMO=1` to accept the exposure deliberately.
...
`RECONBREAKS_PROFILE` is read once, in `config.py`, and it has three states:
...
`RECONBREAKS_AUDIT_ANCHOR` (read by `audit_anchor_path` in
`config/settings.yaml`) to a file on a DIFFERENT volume from
`RECONBREAKS_AUDIT_PATH`, ideally writable by a different principal. This is
not decoration:
