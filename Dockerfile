# Reconciliation Breaks Engine (F1) serving image.
#
...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    RECONBREAKS_PROFILE=gcp \
    RECONBREAKS_SETTINGS=/app/config/settings.yaml \
    PORT=8080
...
CMD ["sh", "-c", "python -m recon_breaks_engine.managed_readiness && exec uvicorn recon_breaks_engine.api.app:app --host 0.0.0.0 --port ${PORT:-8080}"]
