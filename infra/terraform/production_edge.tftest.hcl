...
run "edge_with_routing_stated_off_states_the_switch_and_still_needs_a_console" {
  command = plan
...
run "reject_edge_with_routing_off_and_no_console" {
  command = plan
  ...
  # The case engine opens escalation cases on the same console, so switching routing off does
  # not make the console optional here.
  expect_failures = [
    var.human_review_url,
    check.managed_profile_is_implemented_before_serving,
  ]
